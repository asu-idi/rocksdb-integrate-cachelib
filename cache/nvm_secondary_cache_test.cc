...
class NVMSecondaryCacheTest : public testing::Test {
 ...
 protected:
  ...
  void FailsTest(bool sec_cache_is_compressed) {
    ...
    ASSERT_NOK(
        cache->Insert("k3", &item3, &NVMSecondaryCacheTest::helper_fail_));
  ...
  void BasicIntegrationTest(bool sec_cache_is_compressed) {
    ...
    // k2
    ASSERT_OK(cache->Insert("k3", item3, &NVMSecondaryCacheTest::helper_,
                            str3.length()));

    Cache::Handle* handle;
    handle =
        cache->Lookup("k3", &NVMSecondaryCacheTest::helper_, test_item_creator,
    ...
    // Lookup an non-existent key.
    handle =
        cache->Lookup("k0", &NVMSecondaryCacheTest::helper_, test_item_creator,
                      Cache::Priority::LOW, true, stats.get());
    ...
    handle =
        cache->Lookup("k1", &NVMSecondaryCacheTest::helper_, test_item_creator,
                      Cache::Priority::LOW, true, stats.get());
    ASSERT_NE(handle, nullptr);
    ...
    handle =
        cache->Lookup("k2", &NVMSecondaryCacheTest::helper_, test_item_creator,
                      Cache::Priority::LOW, true, stats.get());
    ASSERT_EQ(handle, nullptr);
  ...
  void BasicIntegrationFailTest(bool sec_cache_is_compressed) {
    ...
    ASSERT_OK(cache->Insert("k1", item1.get(), &NVMSecondaryCacheTest::helper_,
                            str1.length()));
    ...
    ASSERT_EQ(handle, nullptr);
    handle = cache->Lookup("k2", &NVMSecondaryCacheTest::helper_,
  ...
  void IntegrationSaveFailTest(bool sec_cache_is_compressed) {
    ...
    std::string str1 = rnd.RandomString(1020);
    TestItem* item1 = new TestItem(str1.data(), str1.length());
    ASSERT_OK(cache->Insert("k1", item1, &NVMSecondaryCacheTest::helper_fail_,
                            str1.length()));
    std::string str2 = rnd.RandomString(1020);
    TestItem* item2 = new TestItem(str2.data(), str2.length());
    // k1 should be demoted to the secondary cache.
    ASSERT_OK(cache->Insert("k2", item2, &NVMSecondaryCacheTest::helper_fail_,
                            str2.length()));

    Cache::Handle* handle;
    handle = cache->Lookup("k2", &NVMSecondaryCacheTest::helper_fail_,
                           test_item_creator, Cache::Priority::LOW, true);
    ASSERT_NE(handle, nullptr);
    cache->Release(handle);
    // This lookup should fail, since k1 demotion would have failed
    handle = cache->Lookup("k1", &NVMSecondaryCacheTest::helper_fail_,
                           test_item_creator, Cache::Priority::LOW, true);
    ASSERT_EQ(handle, nullptr);
  ...
  void IntegrationCreateFailTest(bool sec_cache_is_compressed) {
    ...
    handle = cache->Lookup("k1", &NVMSecondaryCacheTest::helper_,
                           test_item_creator, Cache::Priority::LOW, true);
    ASSERT_EQ(handle, nullptr);
    // Since k1 didn't get promoted, k2 should still be in cache
    handle = cache->Lookup("k2", &NVMSecondaryCacheTest::helper_,
  ...
  void IntegrationFullCapacityTest(bool sec_cache_is_compressed) {
    ...
    handle2 = cache->Lookup("k1", &NVMSecondaryCacheTest::helper_,
                            test_item_creator, Cache::Priority::LOW, true);
    ASSERT_NE(handle2, nullptr);
...
TEST_F(NVMSecondaryCacheTest, BasicTestWithNoCompression) {
  BasicTest(false, false);
}

TEST_F(NVMSecondaryCacheTest, BasicTestWithMemoryAllocatorAndNoCompression) {
  BasicTest(false, true);
}

TEST_F(NVMSecondaryCacheTest, BasicTestWithCompression) {
  BasicTest(true, false);
}

TEST_F(NVMSecondaryCacheTest, BasicTestWithMemoryAllocatorAndCompression) {
  BasicTest(true, true);
}

TEST_F(NVMSecondaryCacheTest, FailsTestWithNoCompression) { FailsTest(false); }

TEST_F(NVMSecondaryCacheTest, FailsTestWithCompression) { FailsTest(true); }

TEST_F(NVMSecondaryCacheTest, BasicIntegrationTestWithNoCompression) {
  BasicIntegrationTest(false);
}

TEST_F(NVMSecondaryCacheTest, BasicIntegrationTestWithCompression) {
  BasicIntegrationTest(true);
}

TEST_F(NVMSecondaryCacheTest, BasicIntegrationFailTestWithNoCompression) {
  BasicIntegrationFailTest(false);
}

TEST_F(NVMSecondaryCacheTest, BasicIntegrationFailTestWithCompression) {
  BasicIntegrationFailTest(true);
}

TEST_F(NVMSecondaryCacheTest, IntegrationSaveFailTestWithNoCompression) {
  IntegrationSaveFailTest(false);
}

TEST_F(NVMSecondaryCacheTest, IntegrationSaveFailTestWithCompression) {
  IntegrationSaveFailTest(true);
}

TEST_F(NVMSecondaryCacheTest, IntegrationCreateFailTestWithNoCompression) {
  IntegrationCreateFailTest(false);
}

TEST_F(NVMSecondaryCacheTest, IntegrationCreateFailTestWithCompression) {
  IntegrationCreateFailTest(true);
}

TEST_F(NVMSecondaryCacheTest, IntegrationFullCapacityTestWithNoCompression) {
  IntegrationFullCapacityTest(false);
}

TEST_F(NVMSecondaryCacheTest, IntegrationFullCapacityTestWithCompression) {
