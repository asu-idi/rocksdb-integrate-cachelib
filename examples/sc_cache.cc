...
int main(){
    ...
    std::shared_ptr<SecondaryCache> secondary_cache = rocksdb::NewLRUSecondaryCache(2048*1024);
    ...
    std::cout << secondary_cache->num_inserts()<<","<<secondary_cache->num_lookups() <<std::endl;
    Compact(db,"a","z");
    std::cout << secondary_cache->num_inserts()<<","<<secondary_cache->num_lookups() <<std::endl;
